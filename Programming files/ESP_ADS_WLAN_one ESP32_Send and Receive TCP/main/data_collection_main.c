...
7. Create task for tcp_conn() that will setup WIFI connection, ESP32 TCP/IP server
8. Inside the tcp_conn(), create send_data task and receive_data task with receive_data task will be in suspended state 
...
static void tcp_conn(void *pvParameters)
{
    while (1) {
	  ...
	  if (rx_task == NULL) {
            if (pdPASS != xTaskCreate(&recv_data, "recv_data", 4096, NULL, 5, &rx_task)) {
                ESP_LOGE(TAG, "recv task create fail!");
            }
        }
/*	  vTaskSuspend(rx_task);*/
...
void app_main(void)
{
    ...
    xTaskCreate(ADS1256_Collect, "ADS1256_Collect", 10000, (void *) spi, 7, NULL);
   

    init_timer(4000);

    xTaskCreate(&tcp_conn, "tcp_conn", 4096, NULL, 6, NULL);
}
