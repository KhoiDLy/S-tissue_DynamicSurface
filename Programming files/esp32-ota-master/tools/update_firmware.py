...
class UpdateFirmware(object):
    ...
    def run(self):
        ...
        print(result1)
        if result1 != b'OK\r\n':
            raise RuntimeError("failed: ", result1)
            
        result2 = self.send_file()
        print(result2)
        if result2 != b'OK\r\n':
            raise RuntimeError("failed: ", result2)
    ...
    def connect(self):
        ...
        self.socket.connect((self.targetIpAddress, 5100))
        ...
        with open(self.binFilePath, "rb") as f:
            ...
            while True:
                ...
                if chunk:
                    ...
                    h = ("!%04x" % (len(chunk) + 5)) + chunk.decode('latin-1')
...
#            print(cmd)
            sent = self.socket.send(cmd[totalSent:].encode('latin-1')) #modify send here
            if sent == 0:
