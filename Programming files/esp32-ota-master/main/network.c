...
#define EXAMPLE_ESP_WIFI_SSID      "ESP32 2.4"
#define EXAMPLE_ESP_WIFI_PASS      "Cao5566thU"
...
void networkConnect()
{
    ESP_LOGI(TAG, "networkConnect %s", EXAMPLE_ESP_WIFI_SSID);
...
static void networkTask(void *pvParameters)
{
    ...
    while (1) {
        ...
        int b = bind(server_socket, (struct sockaddr *)&serverAddr, sizeof(struct sockaddr_in));
        if (b < 0) {
            ESP_LOGE(TAG, "networkTask: failed to bind socket %d: %d (%s)", server_socket, errno, strerror(errno));
            vTaskDelay(1000 / portTICK_RATE_MS);
            continue;
        }
        ...
        ESP_LOGD(TAG, "networkTask: 'listen' on socket %d", server_socket);
        listen(server_socket, 1); // backlog max. 1 connection
        while (1) {
            ...
            ESP_LOGD(TAG, "--------------------");
            ESP_LOGD(TAG, "networkTask: 'accept' on socket %d", server_socket);
            struct sockaddr_in clientAddr;
            socklen_t clen = sizeof(clientAddr);
            client_socket = accept(server_socket, (struct sockaddr *)&clientAddr, &clen);
            if (client_socket < 0) {
            ...
            do {
                //ESP_LOGD(TAG, "networkTask: waiting for data on socket %d...", client_socket);
                ...
                // Read the request and store it in the allocated bufffer.
                
                int totalRequestLen = 0;
                TNetworkResult result = networkReceive(client_socket, requestBuf, maxRequestLen, &totalRequestLen);
                
                if (result != NWR_READ_COMPLETE) {
                    ESP_LOGI(TAG, "nothing more to, closing socket %d", client_socket);
                    free(requestBuf);
                    close(client_socket);
                    break;
                }
                ...
                int nofWritten = 0;
                ESP_LOGD(TAG, "networkTask: write %d bytes to socket %d: %02x %02x %02x %02x ... | %c%c%c%c...", totalLen, client_socket,
                ...
                do {
                    int n = write(client_socket, &responseBuf[nofWritten], totalLen - nofWritten);
                    int e = errno;
                    //ESP_LOGD(TAG, "networkTask: write: socket %d, n = %d, errno = %d", client_socket, n, e);
        ...
        do {
            ...
            if (n > 0) {
                ...
                if (totalLen > 0) {
                    ...
                    ESP_LOGI(TAG, "networkReceive: recordWithLengthIndicator = %d, expected length = %d, current length = %d", recordWithLengthIndicator, recordLen, totalLen);
			
                    if (  (recordWithLengthIndicator && (totalLen == recordLen)) == 1 || (!recordWithLengthIndicator && buf[totalLen - 1] == '\n'))
                    {
